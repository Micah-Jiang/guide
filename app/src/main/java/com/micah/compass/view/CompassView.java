...
public class CompassView extends View {
    ...
    private void set3DMetrix() {
        ...
        mCameraMatrix.preTranslate(-getWidth() >> 1, -getHeight() >> 1);
        //动作之后恢复位置
        mCameraMatrix.postTranslate(getWidth() >> 1, getHeight() >> 1);
    ...
    private void drawCompassDegreeScale() {
        ...
        for (int i = 0; i < 240; i++) {
            ...
            if (i==0||i==60||i==120||i==180){
                mCanvas.drawLine(getWidth() >> 1, mTextHeight+mOutSideRadius-mCircumRadius+10,
                        getWidth() >> 1,  mTextHeight+mOutSideRadius-mCircumRadius+30, mDeepGrayPaint);
            }else{
    ...
    private void drawCompassOutSide() {
        ...
        mCanvas.drawArc((width >> 1) -mOutSideRadius,mTextHeight, (width >> 1) +mOutSideRadius,mTextHeight+mOutSideRadius*2,-80,120,false,mLightGrayPaint);
        mCanvas.drawArc((width >> 1) -mOutSideRadius,mTextHeight, (width >> 1) +mOutSideRadius,mTextHeight+mOutSideRadius*2,40,20,false,mDeepGrayPaint);
        mCanvas.drawArc((width >> 1) -mOutSideRadius,mTextHeight, (width >> 1) +mOutSideRadius,mTextHeight+mOutSideRadius*2,-100,-20,false,mLightGrayPaint);
        mCanvas.drawArc((width >> 1) -mOutSideRadius,mTextHeight, (width >> 1) +mOutSideRadius,mTextHeight+mOutSideRadius*2,-120,-120,false,mDarkRedPaint);
        mCanvas.restore();
    }
    ...
    private void getCameraTranslate(MotionEvent event) {
        float translateX = (event.getX() - (getWidth() >> 1));
        float translateY = (event.getY() - (getHeight() >> 1));
        //求出此时位移的大小与半径之比
    ...
    private void getCameraRotate(MotionEvent event) {
        float mRotateX = -(event.getY()- ((getHeight()) >> 1));
        float mRotateY = (event.getX()- (getWidth() >> 1));
        //求出旋转大小与半径之比
